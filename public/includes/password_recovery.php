...
{
    $email = recovery_setting_get($pdo, 'recovery_email', '');
...
function recovery_recipient_emails(PDO $pdo): array
{
    ...
    if ($protected !== '' && protected_recovery_enabled($pdo)) {
        $recipients[strtolower($protected)] = $protected;
    }

    $userEmail = recovery_user_email($pdo);
    if ($userEmail !== '') {
        $recipients[strtolower($userEmail)] = $userEmail;
    }
...
{
    $stmt = $pdo->query(
        'SELECT id, username FROM admins WHERE is_active = 1 ORDER BY id ASC LIMIT 1'
    );
    $row = $stmt->fetch();
...
function password_reset_rate_limit_message(PDO $pdo): ?string
{
    ...
    $windowMinutes = PASSWORD_RESET_RATE_WINDOW_MINUTES;
    $stmt = $pdo->prepare(
    ...
    try {
        ...
        $stmt = $pdo->prepare(
            "INSERT INTO password_reset_codes
                (admin_id, code_hash, request_ip_hash, expires_at)
...
function password_reset_current_row(PDO $pdo, int $resetId): ?array
{
    if ($resetId <= 0 || !password_reset_table_exists($pdo)) {
        return null;
    }
...
{
    $row = password_reset_current_row($pdo, $resetId);
    ...
    if (!preg_match('/^\d{6}$/', $code) || !password_verify($code, (string)$row['code_hash'])) {
...
function password_reset_complete(PDO $pdo, int $adminId, string $newPassword): void
{
    ...
    try {
        ...
        if ($stmt->rowCount() !== 1) {
            throw new RuntimeException('Llogaria admin nuk u gjet ose nuk është aktive.');
        }
